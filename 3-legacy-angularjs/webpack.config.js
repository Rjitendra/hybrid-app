...
module.exports = {
  ...
  output: {
    filename: "app.module.js", // Name of the bundled output JavaScript file
    path: path.resolve(__dirname, "./../1-legacy-build/legacy"), // Path where the bundled files will be saved
    clean: true, // Cleans the output folder (dist) before each build
  },

  // Module resolution settings
  resolve: {
    extensions: [".ts", ".js", ".html"], // Automatically resolve these extensions for imports
  },
        ...
        use: [
          {
            ...
            options: {
              minimize: true,
              esModule: false, // Important for AngularJS templates
            },
          },
        ],
      },
  ...
  plugins: [
    ...
    new CopyPlugin({
      patterns: [
        {
          from: "**/*.html",
          to: "[path][name][ext]", // Preserves folders like views/product.list.html
          context: "src/app", // start matching from here
          globOptions: {
            ignore: ["**/index.html"], // Optional: ignore main index.html
          },
        },
      ],
    }),
  ...
  devServer: {
    static: {
      ...
      publicPath: "/", // Files should be accessible from the root
      watch: true,
    },
